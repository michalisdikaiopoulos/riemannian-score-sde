...
- `score_sde/models/flow.py` — the safety mechanisms themselves: `make_early_window_kernel_repulsion_score_fn`, `make_full_window_scaled_kernel_repulsion_score_fn`, `make_late_window_kernel_repulsion_score_fn` (score-correction methods that wrap the learned score with a repulsion term near the unsafe region), and `sample_safe_noise` (the noise-rejection method, which biases the reverse SDE's starting points instead of correcting the score). Dispatch between them happens in `SDEPushForward.get_sampler`.
